...
import { createGroupAction } from '~/lib/admin-group-actions';
...
export function CreateGroupModal() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      setError(null);
      const result = await createGroupAction(trimmedName, description.trim() || null);
      if (result.ok) {
        close();
  ...
  return (
    <>
      ...
      >
        <h2 id="create-group-title" className="mb-4 text-lg font-semibold">
          Criar grupo
        </h2>
