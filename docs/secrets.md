...
| `R2_DOCUMENTS_BUCKET`                                         | api                      | Railway                              | `apps/api/.env`                          | Sem default. Bucket R2 privado de documentos de identidade. Obrigatório sempre que o R2 está configurado. A API não sobe sem essa variável.         |
| `DOCUMENT_URL_TTL_SECONDS`                                    | api                      | Railway                              | `apps/api/.env`                          | Default `60`. TTL do signed GET de documento.                                                                                                       |
| `PROFILE_GATE_ENABLED`                                        | api                      | Railway                              | `apps/api/.env`                          | Default `false`. Liga os gates de perfil no checkout e na assinatura.                                                                               |
| `PROFILE_GATE_ROLLOUT_PERCENT`                                | api                      | Railway                              | `apps/api/.env`                          | Default `0`. Percentual de usuários sob o gate. Bucket determinístico por `userId`.                                                                 |
...
| `FRIDGE_DEVICE_SECRET`                                        | api (fridge WS)          | Railway                              | `apps/api/.env`                          | 32+ bytes random. Shared secret the ESP32 presents on WS connect and receives inside the `UNLOCK:<secret>` frame. Never sent to frontend or logged. |
| `FRIDGE_UNLOCK_API_KEY`                                       | api (fridge unlock)      | Railway                              | `apps/api/.env`                          | 32+ bytes random. Service key required in `X-API-Key` header on `POST /api/fridge/unlock`. Never sent to frontend or logged.                        |
