...
2. Esperar acúmulo de perfis completos entre os novos cadastros. Ligar o
   gate antes disso bloqueia a base legada inteira de uma vez.
3. `PROFILE_GATE_ENABLED=true` e `PROFILE_GATE_ROLLOUT_PERCENT=5`.
4. Escalar 5 → 25 → 50 → 100, com no mínimo 24 h de observação em cada
   passo.
...
as colunas são nullable e a tabela nova não afeta nenhum caminho existente
